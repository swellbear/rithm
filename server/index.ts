...
// MOBILE LOGIN FIX: Session configuration for persistent authentication
app.use(session({
  secret: process.env.SESSION_SECRET || 'rithm-session-secret-key-2025',
  resave: false,
  ...
  cookie: {
    ...
    sameSite: process.env.NODE_ENV === 'production' ? 'none' : 'lax' // Mobile compatibility
  }
}));

// MOBILE LOGIN FIX: CORS Configuration for mobile browsers
app.use((req, res, next) => {
  const origin = req.headers.origin;
  
  // Allow specific origins with credentials for mobile compatibility
  if (process.env.NODE_ENV === 'production') {
    // Production: allow render domain and mobile browsers
    res.header('Access-Control-Allow-Origin', origin || 'https://rithm-zkfo.onrender.com');
  } else {
